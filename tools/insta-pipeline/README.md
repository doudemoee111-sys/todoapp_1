...
リポジトリの `config.json` は **「AIツール実演 × バックオフィス業務」で作り込み済み**です。別ジャンルでやる場合は同じスキーマで書き換えてください（`config.example.json` が雛形）。

| キー | 役割 |
|---|---|
| `account.genre` / `persona` | 誰に何を届けるか。ペルソナは属性ではなく**「どの作業で何時間困っているか」**で書くと台本の精度が上がる |
| `account.expertise` | 何を根拠に語るか。02の台本生成に渡される |
| `account.tone` / `ngExpressions` / `forbiddenTopics` | 文体と、踏み込まない話題。02・06・07で使われる |
| `post.formatRotation` | **曜日別の投稿フォーマット**（後述）。毎日投稿のマンネリ防止 |
| `post.hookPatterns` | 冒頭3秒の型。02がこの中から選ぶ |
| `post.ctaVariants` | CTAを複数書くと**日替わりでローテーション**される |
| `hashtags.fixed` / `pool` | 固定タグ＋候補プール。AIが勝手にタグを作らないよう**プール内からのみ選択**させる |
| `affiliate.programs` | **案件レジストリ**（後述） |
| `compliance.bannedPatterns` | 禁止表現。**正規表現**で書けるので活用形も拾える |

### 曜日別フォーマット（毎日投稿の骨格）

毎日同じ型で投稿すると必ず飽きられるので、曜日ごとに型と狙う指標を変えています。
...
| 水 | 失敗・注意喚起 | シェア |
| 木 | 数字・損益分岐 | 保存 |
| 金 | 逆張り（任せない方がいい） | シェア・コメント |
| 土 | チェックリスト | 保存 |
| 日 | 質問回答・小ネタ | 関係性 |

`topics/backlog.md` のネタは `- [wed] AIに契約書を読ませたら見落としが3つ出た話` のように**曜日タグ付き**で書きます。01はその日の型に合うネタを優先して選び、在庫が切れたらその型に合うネタだけを14件補充します。初期在庫は42本（約6週間分）入っています。

### 案件レジストリ（ASP規約違反の事前検出）

`affiliate.programs` に紹介予定の案件を登録しておくと、07が本文を照合します。

| `instagramAllowed` | 挙動 |
|---|---|
| `true` | 通過 |
| `false` | **block**（Instagram掲載不可の案件） |
| `null`（未確認） | **block**（確認を強制する） |

デフォルトは `null` です。**ASPの管理画面で「SNS可 / Instagram掲載可」を自分で確認してから** `true`/`false` を入れてください。ここを空のままにしておくと、提携解除のリスクをコードが検知できません。
