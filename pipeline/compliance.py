...
NG_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"効果があ(る|ります)", "効果の断定"),
    # 終止形・連体形も見る。「解消します」は捕まえていたのに「解消する3ステップ」は
    # 素通りしていた。題名は連体形で終わることが多いので、辞書として片手落ちだった。
    (r"(解消|根治|克服)(します|できます|する|できる|した)", "効果の断定"),
    # 結果の約束のうち、動詞が「変わる」でない型。「眠れる夜を取り戻す」は
    # 辞書を全て通過して自動生成の題名に現れた。しかもその回の概要欄には
    # 有料講座の広告が入っており、景表法（優良誤認）の側からも危うい。
    (r"(眠れる夜|静けさ|睡眠|眠り|日常)を取り戻(す|せ|し)", "結果の約束"),
    ...
    # 名乗れない専門性を名乗る型。このチャンネルには資格者も監修者もいない。
    # 「プロが教える」は、辞書を全て通過したうえで自動生成のタイトルに現れた。
    # 薬機法より景表法（優良誤認）の問題で、そもそも事実として偽り。
    (r"(プロ|専門家|医師|薬剤師|専門医)が(教える|伝える|解説)", "名乗れない専門性の僭称"),
    (r"(睡眠|快眠|health)の?プロ(が|は|に)", "名乗れない専門性の僭称"),
    (r"当院|専門家監修|医師監修", "監修者がいないのに監修を名乗る表現"),
    # 体験談。薬機法では効果の証明に体験談を用いることが問題とされるが、
    # このチャンネルではもう一段手前で問題になる——語りは生成されたもので、
    # 体験した人がいない。「実際の体験談を元に」は、事実として偽り。
    # 「経験がある人もいます」のような一般化した記述は捕まえない。
    (r"(実際の|私の|筆者の)?体験談", "体験した人がいない（語りは生成）／効果の証明に使えない"),
    # 一人称を三人称に置き換えただけの体験談。「知人の体験と選択」が、
    # 体験談を禁じた1週間後に目次へ現れた。このチャンネルに知人はいない。
    # 出典のある症例報告と区別するため、人間関係を指す語に限定して捕まえる。
    # 「知人に相談する」のような助言は通す。
    ...
    # 「これで安心」型。医療広告ガイドラインは治療に関する「安心」「安全」を
    # 根拠なく掲げることを問題にしており、この形は結果の約束でもある。
    # 「安心して見られる内容です」「安心してご相談ください」のような、
    # 読み手の構えについての言い方は妨げないよう、決め文句の形に限定する。
    (r"(家族|あなた|これで|もう|今夜から)も?安心", "結果の約束（根拠のない『安心』）"),
    # 存在しない学会名。このチャンネルの書き方は「出典を示して具体的に語る」
    # ことで断定を避ける構えなので、その出典の名前が間違っていると、避けている
    # はずの信頼性の問題がそのまま戻ってくる。実際に「アメリカ小児学会」
    # （正しくは米国小児科学会 / American Academy of Pediatrics）が概要欄に出た。
    # 「科」が落ちた形だけを狭く捕まえる。正しい名前は妨げない。
    (r"(アメリカ|米国|日本)小児学会", "学会名が不正確（正しくは小児科学会）"),
    (r"(アメリカ|米国|日本)耳鼻科学会", "学会名が不正確（正しくは耳鼻咽喉科系の学会名）"),
    (r"日本睡眠医学会", "学会名が不正確（日本の学会は日本睡眠学会）"),
...
- 名乗れない専門性: 「プロが教える」「専門家が教える」「医師監修」。このチャンネルに
  資格者も監修者もいない。事実として偽りになる。
- 体験談: 「実際の体験談」「使ってみた結果」。語りは生成されたもので、体験した人が
  いない。存在しない人の経験を語らないこと。「〜という人もいます」「〜という報告が
  あります」のように、出典のある一般化した書き方にすること。
  **主語を他人に変えても同じ。** 「知人の体験」「友人のケース」「ある男性の話」も
  不可。このチャンネルに知人も友人もいない。章の見出しでも本文でも使わないこと。
  症例に触れるなら「〜学会の資料で報告されている例では」のように出典を示す。
  **ただし学会名は正確に。** 曖昧なら団体名を出さず「複数の研究で」と書く。
  存在しない団体を出典として挙げるのは、断定を避けるための出典表示そのものを
  壊す。例: 米国小児科学会（American Academy of Pediatrics）、日本睡眠学会。
